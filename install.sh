...
time=$(date +%H:%M:%S)
echo "[$time] Installing Liquid Galaxy Retro Gaming..." | tee -a /home/lg/lg-retro-gaming/logs/$filename
...
if [ "$DATA" == "" ]; then
    time=$(date +%H:%M:%S)
    echo "[$time] Port 3123 not open, opening port..." | tee -a /home/lg/lg-retro-gaming/logs/$filename
...
cd /home/lg/lg-retro-gaming/server
npm install 2>> ./home/lg/lg-retro-gaming/logs/$filename
cd ..
...
echo $PW | sudo -S pm2 start ./server/index.js --name LGRG_PORT:3123 2>> /home/lg/lg-retro-gaming/logs/$filename
...
time=$(date +%H:%M:%S)
echo "[$time] Installation complete. Reboot machine to finish installation" | tee -a /home/lg/lg-retro-gaming/logs/$filename
